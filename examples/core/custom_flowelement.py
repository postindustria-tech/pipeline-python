...
class AstrologyFlowElement(FlowElement):
    ...
    def __init__(self):
        ...
        self.properties.addProperty("starSign", {
            "type": "string",
            "description": "The user's star sign."
        })
        self.properties.addProperty("getLatitude", {
            "type": "javascript",
    ...
    def processInternal(self, flowData):
        ...
        result["getLatitude"] = "navigator.geolocation.getCurrentPosition(function(position) { document.cookie = 'latitude=' + position.coords.latitude; });"
        ...
        if latitude:
            ...
            result["hemisphere"] = "Northern" if latitude > 0 else "Southern"
...
myPipeline = (PipelineBuilder()).add(AstrologyFlowElement()).build()
...
def server():
    ...
    if flowData.astrology.hemisphere:
        output += "<p>Look at the " + flowData.get("astrology").get("hemisphere") + " hemisphere stars tonight!</p>"
    ...
    for script in javaScript.values():
